...
def simCV(value):
    ...
    Ei = 0.0 #V - Initial voltage
    Emax = 4.0 #V - Max voltage 
    ...
    Cb = np.array([0.00,0.00,0.01,0.00,0.00]) #M - concs. for species X, Y, W, U, Z
    ...
    if freeGrid == 0:
        ...
        if LSweep == 1:
            ...
            cyc_inds = len(Evt_cyc)*np.arange(1,cycles+1,1)
    ...
    print(value,PP,np.round(savExact[0],5),np.round(savExact[2],5),SChemProdMax,maxStepErr,elapsed)
...
def getConstants(F,R,T,optn):
    ...
    if optn == 0: #rate constants
        ...
        k6f = 0.0 #10 ** (10) #1/s - Homog., first order, rxn 2
        k6b = k6f/Kv[5]
...
#valVect = np.arange(-15.0,-16.0,-2.0)
